...
export const acceptFiles = ({ id , type,  data , field }) => {
    ...
    return async dispatch => {
        ...
        try {
            ...
            switch (type) {
                case "copyrights":
                    response = await mainApiInstance.patch(`/api/contracts/contractFiles/${id}/${field}`, data);
                    break;
                case "project":
                    response = await mainApiInstance.patch(`/api/contracts/contractFiles/${id}/${field}` , data);
                    break;
...
export const acceptAllFiles = ({ id, type , data , field }) => {
    ...
    return async dispatch => {
        ...
        try {
            const response = await mainApiInstance.patch(`/api/contracts/contractFiles/${id}/accept-all`, {
                cycle: type
            });
            dispatch({ type: Types.FETCH_DATA_SUCCESS, payload: response.data, req });
        } catch (error) {
