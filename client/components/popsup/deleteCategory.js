...
function DeleteCategory({onClick,id}) {
    const { t } = useTranslation();

    const toggleDirectorConfirmed = () => {
        ClosePopUp("delete-category-team-"+id)
        onClick?.(id)
    }

    return (
        <>
            <Popup id={'delete-category-team-'+id} header={"Delete Category"} >
                <div method="post" className='mt-12 mx-5'>
                    <span className="text-lg font-[#2F2F2F]"> Are you sure you want to delete <span className="text-[#2F2F2F] dark:text-white">{t("this category?")}</span></span>
                    <div className='mt-12'/>
