    ...
    <div className='rounded-[45px] flex flex-col justify-around cardborder-3 bg-white dark:bg-[#1A2024] p-6 pt-16 h-full'>
        ...
        </div>
        {
            activity.map((e, index) => (
                ...
                <div  key={index}>
                    <div className='mb-2'>
                        <span className='text-4xl font-medium'>{convertToK(e.count)} </span>
                        <span className='text-xl capitalize font-medium opacity-50'>{e.title}</span>
                    </div>
                    <div className='flex'>
                        <div className='dashboard_padge text-sm mx-1 px-2 text-DS_black opacity-70'> {e.state} </div>
                    </div>
                </div>
                    {index !== activity.length - 1 && <div className='border-b border-black opacity-15'></div>}
                </>
            ))
        }
    </div>
