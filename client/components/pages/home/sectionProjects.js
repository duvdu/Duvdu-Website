...
function SectionProjects({ projects , inclusive }) {
        ...
        <div>
            <div className="sm:hidden grid minmax-150 gap-3">
                {repeatedProjects?.map((item, i) => (
                    item &&
                    <SmallProjectItem key={item?._id} inclusive={inclusive} cardData={item} className={i % 3 === 0 ? 'col-span-2 row-span-2' : ''} isbig={i % 3 === 0} />
                ))}
            </div>
